...
void ANBGGameModeBase::OnPostLogin(AController* NewPlayer)
{
	...
	// {
	// 	NBGGameStateBase->MulticastRPCBroadcastLoginMessage(TEXT("XXXXXXX"));
	// }
	// 
	...
	if (IsValid(NBGPlayerController) == true)
	{
		...
		ANBGGameStateBase* NBGXGameStateBase =  GetGameState<ANBGGameStateBase>();
		if (IsValid(NBGXGameStateBase) == true)
		{
			NBGXGameStateBase->MulticastRPCBroadcastLoginMessage(NBGPS->PlayerNameString);
		}
	}
}
...
void ANBGGameModeBase::JudgeGame(ANBGPlayerController* InChattingPlayerController, int InStrikeCount)
{
	if (3 == InStrikeCount)
	{
		...
		for (const auto& NBGPlayerController : AllPlayerControllers)
		{
			if (IsValid(NBGPS) == true)
			{
				FString CombinedMessageString = NBGPS->PlayerNameString + TEXT(" has won the game.");
				NBGPlayerController->NotificationText = FText::FromString(CombinedMessageString);
